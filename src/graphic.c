...
/*   Updated: 2024/04/22 10:30:56 by abasdere         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_keys(t_data *data, const char *k)
{
	...
	cpy = (t_graphic){data->map.graphic.color, data->map.graphic.texture,
		data->map.graphic.wall_state, NULL, {0}};
	while (cpy.color || cpy.texture)
	{
		if ((cpy.color
				&& !ft_strncmp(cpy.color->key, k, ft_strlen(cpy.color->key)))
			|| (cpy.texture
				&& !ft_strncmp(cpy.texture->key, k,
					ft_strlen(cpy.texture->key))))
...
static void	dispatch(t_data *data, const char **kvsp)
{
	...
	l = ft_strlen(kvsp[0]);
	if (kvsp[2] && l >= 4 && kvsp[0][l - 4] == '.' && kvsp[0][l - 3] == 'x'
		&& kvsp[0][l - 2] == 'p' && kvsp[0][l - 1] == 'm')
		return (add_sprite(data, kvsp));
	else if (kvsp[2])
		cerror(NB_ERROR, kvsp[0], data->collector);
	(check_keys(data, kvsp[0]), l = ft_strlen(kvsp[1]));
	if (l >= 4 && kvsp[1][l - 4] == '.' && kvsp[1][l - 3] == 'x'
